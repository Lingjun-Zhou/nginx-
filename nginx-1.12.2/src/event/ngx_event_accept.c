...
#if (NGX_HAVE_ACCEPT4)
        if (use_accept4) {
			//第一个参数为监听套接字，在调用listen函数之后，一个套接字会从主动连接的套接字变身为一个监听套接字
			//lc->fd包含服务器的ip和port信息
			//accept返回是一个连接套接字，它代表着一个网络已经存在的点点连接。
			//之后读写事件的fd都是指这个新的socket s
            s = accept4(lc->fd, &sa.sockaddr, &socklen, SOCK_NONBLOCK);
        } else {
            s = accept(lc->fd, &sa.sockaddr, &socklen);
        }
#else
        s = accept(lc->fd, &sa.sockaddr, &socklen);// accept一个新的连接
#endif

        if (s == (ngx_socket_t) -1) { //连接的错误处理
