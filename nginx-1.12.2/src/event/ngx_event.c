...
ngx_module_t  ngx_event_core_module = {
    ...
    ngx_event_process_init,                /* init process 通过这个回调函数，进行event的初始化。*/
    NULL,                                  /* init thread */
    NULL,                                  /* exit thread */
    NULL,                                  /* exit process */
    NULL,                                  /* exit master */
...
// 获取相应模块的配置结构
 ...
 // 判断是否使用mutex锁，主要是为了控制负载均衡。ccf->master主要确定下是否是master-worker模式。单进程模式就不需要进行下面操作了。/* 多进程情况下，进程数大于1的情况，使用accept_mutex锁。惊群的时候需要用到。*/
    if (ccf->master && ccf->worker_processes > 1 && ecf->accept_mutex) {
        ngx_use_accept_mutex = 1;//使用mutex控制进程的负载均衡
...
#endif
//创建连接池。现在已经是在worker中了，所以每个worker都有自己的connection数组.分配一块内存，存储连接
    cycle->connections =
...
//创建所有读事件,分配一块内存，存放读取事件
    ...
    for (i = 0; i < cycle->connection_n; i++) {//初始化读事件
        rev[i].closed = 1;
        rev[i].instance = 1;//防止stale event
    }
 //创建写事件, 分配一块内存，存储写入事件
    ...
    for (i = 0; i < cycle->connection_n; i++) {//初始化写事件
        wev[i].closed = 1;
    }
 ...
 //初始化连接池,初始化连接数据
	...
	//链表
        c[i].data = next;
        c[i].read = &cycle->read_events[i];//每一个连接的读写事件对应cycle的读写事件
        c[i].write = &cycle->write_events[i];//每一个连接的读写事件对应cycle的读写事件
...
//设置listen句柄的事件回调，这个回调里面会accept，然后进行后续处理，这个函数是nginx事件驱动的第一个函数
        rev->handler = (c->type == SOCK_STREAM) ? ngx_event_accept//SOCK_STREAM表示TCP连接
